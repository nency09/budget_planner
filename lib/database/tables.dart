...
// Keep schemaVersionGlobal at 46 to match generated schema_versions.dart.
// New tables like MerchantCategoryCache and AIWeeklyInsights are created
// defensively in beforeOpen(), so existing users are upgraded safely.
...
// AI Feature Tables - Phase 1 Implementation

/// Merchant category cache for smart transaction categorization
/// Stores AI-predicted categories for merchants to avoid repeated API calls
@DataClassName('MerchantCategoryCacheEntry')
class MerchantCategoryCache extends Table {
  TextColumn get merchantName => text()(); // Primary key - merchant name (normalized)
  TextColumn get predictedCategory => text()(); // Category name from AI
  RealColumn get confidence => real().withDefault(const Constant(1.0))(); // 0.0 to 1.0
  ...
  @override
  Set<Column> get primaryKey => {merchantName};
}

/// Weekly AI insights cache
/// Stores weekly insights generated by AI to avoid regenerating every time
@DataClassName('AIWeeklyInsight')
class AIWeeklyInsights extends Table {
  TextColumn get insightPk => text().clientDefault(() => uuid.v4())();
  DateTimeColumn get weekStart => dateTime()(); // Start of the week (Monday)
  DateTimeColumn get weekEnd => dateTime()(); // End of the week (Sunday)
  TextColumn get insightText => text()(); // The generated insight text
  DateTimeColumn get createdAt =>
      dateTime().clientDefault(() => DateTime.now())();
...
class FinanceDatabase extends _$FinanceDatabase {
  ...
  MigrationStrategy get migration {
    return MigrationStrategy(
      ...
      beforeOpen: (details) async {
        ...
        // Ensure new AI-related tables exist even on older databases.
        // This is safe to run multiple times because createTable is wrapped in try/catch.
        try {
          final m = createMigrator();
          await m.createTable($MerchantCategoryCacheTable(database));
        } catch (e) {
  ...
  // ---------------------------------------------------------------------------
  // AI Feature Database Methods - Phase 1
  // ---------------------------------------------------------------------------

  /// Get merchant category cache entry
  Future<MerchantCategoryCacheEntry?> getMerchantCategoryCache(String merchantName) async {
    final normalized = merchantName.trim().toLowerCase();
    return (select(merchantCategoryCache)
          ..where((t) => t.merchantName.equals(normalized)))
        .getSingleOrNull();
  }

  /// Create or update merchant category cache
  ...
  /// Get weekly AI insight for a specific week
  ...
  /// Create or update weekly AI insight
